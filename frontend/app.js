...
function loadLayoutAndBookings() {
    ...
    db.collection('tickets').onSnapshot(snapshot => {
        ...
        snapshot.forEach(doc => {
            const data = doc.data();
            const isUnexpiredPending = data.status === 'EMAIL_BESTAETIGUNG_AUSSTEHEND' &&
                (!data.expiresAt || Date.parse(data.expiresAt) > now);
            const isActive = data.status === 'GÜLTIG' || data.status === 'ENTWERTET' || isUnexpiredPending;

            if (isActive && data.seat) bookedSeats.push(data.seat);

            const tr = document.createElement('tr');
            const canAdminCancel = ['GÜLTIG', 'ENTWERTET'].includes(data.status);
            tr.innerHTML = `
                <td>${doc.id}</td>
                <td>${data.name || ''}</td>
                <td>${data.email || ''}</td>
                ...
                <td>
                    ${canAdminCancel
                        ? `<button class="btn-secondary admin-cancel-ticket-btn" data-ticket-id="${doc.id}" style="background:#d32f2f;color:#fff;">❌ Stornieren</button>`
                        : '<span style="color:#777;font-size:0.9em;">Keine Admin-Stornierung möglich</span>'}
                </td>
            `;

            const cancelBtn = tr.querySelector('.admin-cancel-ticket-btn');
            if (cancelBtn) cancelBtn.addEventListener('click', () => adminCancelTicket(doc.id, data));
...
async function adminCancelTicket(ticketId, ticket) {
    const reasons = [
        'Veranstaltungsorganisation / Änderungen',
        'Veranstaltung kann nicht wie geplant stattfinden',
        'Sitzplatz / Saalplan musste geändert werden',
    ...
    const selected = prompt(
        `Stornierungsgründe für Ticket ${ticketId}:\n\n` +
        reasons.map((reason, index) => `${index + 1}. ${reason}`).join('\n') +
        '\n\nMehrere Gründe möglich: Nummern mit Komma trennen, z. B. 1,3,5.'
    );
    if (selected === null) return;

    const indexes = [...new Set(
        selected.split(',').map(v => parseInt(v.trim(), 10)).filter(n => n >= 1 && n <= reasons.length)
    )];
    if (!indexes.length) {
        alert('❌ Bitte mindestens einen gültigen Stornierungsgrund auswählen.');
        return;
    }

    const selectedReasons = indexes.map(index => reasons[index - 1]);
    const confirmation = confirm(
        `Ticket ${ticketId} (${ticket.name || 'Gast'}, Platz ${ticket.seat || '-'}) wirklich stornieren?\n\n` +
        `Es wird automatisch eine zweite E-Mail an ${ticket.email || 'die hinterlegte Adresse'} gesendet.\n\n` +
    ...
    try {
        ...
        const response = await fetch(`${BACKEND_BASE_URL}/admin-cancel-ticket`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            },
            body: JSON.stringify({ ticketId, reasons: selectedReasons })
        });
        const data = await response.json().catch(() => ({}));
        if (!response.ok && response.status !== 207) throw new Error(data.error || 'Ticket konnte nicht storniert werden.');

        if (data.emailSent === false) {
            alert(`⚠️ Ticket wurde storniert, aber die zweite E-Mail konnte nicht versendet werden.\n\n${data.warning || ''}`);
        } else {
            alert('✅ Ticket wurde storniert und die zweite E-Mail mit den Stornierungsgründen wurde versendet.');
        }
    } catch (err) {
        alert('❌ ' + err.message);
    }
...
async function handleCancellationLink() {
    ...
    const token = params.get('cancelToken');
    if (!token) return;

    window.history.replaceState({}, document.title, window.location.pathname);

    if (!confirm('Möchtest du dieses Ticket wirklich stornieren? Der Sitzplatz wird danach wieder freigegeben.')) return;

    try {
        const response = await fetch(`${BACKEND_BASE_URL}/cancel-ticket`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token })
        });
        const data = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(data.error || 'Die Stornierung ist fehlgeschlagen.');
...
async function sendAdminEmail() {
    const user = auth.currentUser;
    if (!user) throw new Error('Bitte zuerst als Admin einloggen.');

    const token = await user.getIdToken(true);
    const response = await fetch(`${BACKEND_BASE_URL}/admin-send-email`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
        ...
        body: JSON.stringify({
            to: document.getElementById('admin-mail-to').value.trim(),
            subject: document.getElementById('admin-mail-subject').value.trim(),
            html: document.getElementById('admin-mail-body').value
                .replace(/&/g, '&amp;')
                .replace(/</g, '&lt;')
                .replace(/>/g, '&gt;')
                .replace(/\n/g, '<br>')
...
async function sendTicketEmail(userEmail, userName, seatNumber, ticketId) {
    const BACKEND_URL = `${BACKEND_BASE_URL}/send-ticket`;
...
async function finishVerifiedBooking(ticketId, user) {
    ...
    if (ticket.expiresAt && Date.parse(ticket.expiresAt) <= Date.now()) {
        ...
        throw new Error('Die 5-Minuten-Reservierung ist abgelaufen. Bitte wähle den Platz erneut.');
    }
...
async function sendEmailVerificationForBooking(email, name, seat) {
    ...
    if (!existingTicket.empty) {
        const activeTicket = existingTicket.docs.find(doc => {
            const data = doc.data();
            if (data.status === 'STORNIERT' || data.status === 'ABGELAUFEN') return false;
            if (data.status === 'EMAIL_BESTAETIGUNG_AUSSTEHEND' && data.expiresAt && Date.parse(data.expiresAt) <= Date.now()) return false;
            return true;
        });
    ...
    const response = await fetch(`${BACKEND_BASE_URL}/send-verification`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            email,
    ...
    alert(
        ...
        `Der Platz ${seat} ist für 5 Minuten vorgemerkt. Ohne E-Mail-Bestätigung wird die Reservierung automatisch freigegeben.`
    );
}
...
document.getElementById("booking-form").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        if (
            ...
            const activeExisting = existingTicket.docs.find(doc => {
                const data = doc.data();
                if (data.status === 'STORNIERT' || data.status === 'ABGELAUFEN') return false;
                if (data.status === 'EMAIL_BESTAETIGUNG_AUSSTEHEND' && data.expiresAt && Date.parse(data.expiresAt) <= Date.now()) return false;
                return true;
